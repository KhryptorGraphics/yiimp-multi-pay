...
class db_coins extends CActiveRecord
{
	...
	public function deleteDeps()
	{
		...
		dborun("DELETE FROM account_balances WHERE account_id IN $ids_query OR coinid=".$coin->id);
		dborun("DELETE FROM account_addresses WHERE account_id IN $ids_query OR coinid=".$coin->id);
